-- Backup DB initialization (PostgreSQL)
-- Creates schema objects and inserts the same sample data as init.sql (MySQL).
--
-- If you run via psql, you can do:
--   psql -U postgres -f init-backup.sql
--
-- NOTE: CREATE DATABASE cannot run inside a transaction block.

-- Create database (run as a superuser / a role with CREATEDB)
CREATE DATABASE hotel_db_backup;

-- For psql, connect to the new database:
-- \c hotel_db_backup
